...
void show_usage(const char *path) {
  printf("Usage:\n");
  printf("%s [options]\n", path);
  printf("-f <files> test TAD cg_match on the all the files listed.\n");
...
int run_file(const char* filename, bool do_verify)
{
  ...
  if (file != stdin && do_verify)
  {
    ...
    readed = fscanf(file, "%d", &expected_winner);
    if (readed != 1) {
      printf("Error reading expected winner \n");
      exit(EXIT_FAILURE);
    }
  ...
  if (do_verify) {
    ...
    correct = is_match_correct(match) != correct;
  }
  printf("\n");
  // Winner
  ...
  if (do_verify) {
    total_fail = winner != expected_winner;
    print_pass(!total_fail);
    if (total_fail) {
      printf(" [expected: %d]", expected_winner);
    }
